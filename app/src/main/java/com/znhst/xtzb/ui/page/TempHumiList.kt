...
@Composable
fun TempHumiList(deviceViewModel: DeviceViewModel = viewModel(), navController: NavController, category: TempHumiCategory) {
    val context = LocalContext.current
    val deviceList by deviceViewModel.tempHumiList.collectAsState()

    LaunchedEffect(Unit) {
        withContext(Dispatchers.IO) {
            deviceViewModel.fetchTempHumis(category)
        }
    }
        ...
        LazyColumn(modifier = Modifier.padding(start = 16.dp, end = 16.dp)) {
            items(deviceList) { device ->
                Card(modifier = Modifier.fillMaxWidth(),
                    onClick = {
                        navController.navigate("temp_humi_detail/${device.deviceNo}")
                    }) {
                    Row(
                        modifier = Modifier
                            .fillMaxWidth()
